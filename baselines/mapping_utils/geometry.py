...
#     return pointcloud
def cpu_pointcloud_from_array(points, colors):
    # print(f"=== cpu_pointcloud_from_array 调试开始 ===")
    # print(f"输入参数:")
    # print(f"  - points type: {type(points)}, shape: {points.shape if hasattr(points, 'shape') else 'N/A'}")
    # print(f"  - points dtype: {points.dtype if hasattr(points, 'dtype') else 'N/A'}")
    # print(f"  - points min: {points.min() if hasattr(points, 'min') else 'N/A'}")
    # print(f"  - points max: {points.max() if hasattr(points, 'max') else 'N/A'}")
    # print(f"  - colors type: {type(colors)}, shape: {colors.shape if hasattr(colors, 'shape') else 'N/A'}")
    # print(f"  - colors dtype: {colors.dtype if hasattr(colors, 'dtype') else 'N/A'}")
    ...
    if len(points) == 0 or len(colors) == 0:
        # print(f"⚠️ 输入数据为空数组! points长度: {len(points)}, colors长度: {len(colors)}")
        return o3d.geometry.PointCloud()
    
    if len(points) != len(colors):
        # print(f"⚠️ 点数和颜色数不匹配! points: {len(points)}, colors: {len(colors)}")
        # 调整较小的数组
        min_len = min(len(points), len(colors))
        points = points[:min_len]
        colors = colors[:min_len]
        # print(f"  调整后长度: {min_len}")
    
    try:
        # print(f"创建Open3D点云对象...")
        pointcloud = o3d.geometry.PointCloud()
        
        # print(f"设置点坐标...")
        # 检查点坐标数据类型
        if points.dtype != np.float64:
            # print(f"  转换points从 {points.dtype} 到 float64")
            points = points.astype(np.float64)
        pointcloud.points = o3d.utility.Vector3dVector(points)
        # print(f"  设置成功，点数量: {len(pointcloud.points)}")
        
        # print(f"设置点颜色...")
        # 检查颜色数据类型和范围
        if colors.dtype != np.float64:
            # print(f"  转换colors从 {colors.dtype} 到 float64")
            colors = colors.astype(np.float64)
        
        # 确保颜色值在[0, 1]范围内
        if colors.max() > 1.0 or colors.min() < 0.0:
            # print(f"  颜色值超出[0,1]范围: min={colors.min():.3f}, max={colors.max():.3f}")
            colors = np.clip(colors, 0.0, 1.0)
        
        pointcloud.colors = o3d.utility.Vector3dVector(colors)
        # print(f"  设置成功，颜色数量: {len(pointcloud.colors)}")
        
        # print(f"=== cpu_pointcloud_from_array 调试结束 (成功) ===")
        return pointcloud
        
    except Exception as e:
        print(f"❌ cpu_pointcloud_from_array 失败: {e}")
        print(f"=== 详细错误追踪 ===")
        import traceback
        traceback.print_exc()
        print(f"=== cpu_pointcloud_from_array 调试结束 (失败) ===")

...
class InstructNavMapperROS:
    ...
    def update(self, rgb, depth, position, rotation):
        ...
        self.depth_trajectory.append((depth / 10.0 * 255.0).astype(np.uint8))
    ...
    def query_chainon(self) -> Dict[str, Any]:
        ...
        for attempt in range(3):
            try:
                ...
                print(f"GPT-4 Response: {raw_answer}...")
    ...
    def query_gpt4v(self) -> int:
        ...
        for attempt in range(3):
            try:
                ...
                print(f"GPT-4V Response: {raw_answer}...")
    ...
    def plan_navigation(self, instruction: str):
        ...
        try:
            ...
            # Optional: visualize affordance map
            self._display_affordance_map(affordance)
    ...
    def _display_affordance_map(self, affordance):
        """Display affordance map using OpenCV"""
        try:
            # Convert to numpy array
            if hasattr(affordance, 'cpu'):
                affordance_np = affordance.cpu().numpy()
            else:
                affordance_np = affordance
            
            # Normalize to [0, 255]
            if affordance_np.size > 0:
                if affordance_np.max() > affordance_np.min():
                    normalized = ((affordance_np - affordance_np.min()) / 
                                (affordance_np.max() - affordance_np.min()) * 255)
                else:
                    normalized = np.zeros_like(affordance_np) * 255
                ...
                if len(normalized.shape) == 2:
                    ...
                    max_val = affordance_np.max()
                    min_val = affordance_np.min()
                    cv2.putText(colored, f"Max: {max_val:.3f}", (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    cv2.putText(colored, f"Min: {min_val:.3f}", (10, 60), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    
                    cv2.imshow("Affordance Map", colored)
                    cv2.waitKey(1) 
                    
                    print(f"Affordance Map显示: shape={affordance_np.shape}, "
                        f"range=[{min_val:.3f}, {max_val:.3f}]")
                else:
                    print(f"Affordance Map形状不支持显示: {affordance_np.shape}")
        except Exception as e:
            print(f"显示Affordance Map时出错: {e}")
...
class InstructNavBaseline(VLNConnector):
    ...
    def __init__(self):
        ...
        # Remove threading-related variables
    ...
    def control_once_async(self):
        ...
        Now runs inference synchronously instead of in separate thread.
        """
        ...
        # Set planning flag to prevent re-entry
        ...
        try:
            ...
            # Run InstructNav planning synchronously
    ...
    def _mapper_to_ros(self, target_point):
        ...
        if distance > max_distance:
            ...
            ros_x *= scale
            ros_y *= scale
            self.get_logger().warning(f"Limited movement from {distance:.2f}m to {max_distance}m")
    ...
    def destroy_node(self):
        ...
        # Remove threading cleanup since we don't use threads anymore
        super().destroy_node()
...
def main(args=None):
    ...
    try:
        # Main loop - single-threaded execution
        while rclpy.ok():
            # Process ROS2 messages
            rclpy.spin_once(node, timeout_sec=0.05)
            
            # Run inference synchronously in the main thread
